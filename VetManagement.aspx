<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="VetManagement.aspx.cs" Inherits="CatResort.VetManagement" %>

<!DOCTYPE html>
<html xmlns="http://www.w3.org/1999/xhtml">
<head runat="server">
    <title>Vet Management - Cat Resort</title>
    <link href="https://fonts.googleapis.com/css2?family=Recursive:wght@300;400;500;600;700&display=swap" rel="stylesheet">
    <style>
        /* Reuse the same styles from StaffManagement */
        * { margin:0; padding:0; box-sizing:border-box; }
        ...
        .btn-primary, .btn-secondary, .btn-danger, .btn-query { padding:10px 20px; border:none; color:white; font-family:'Recursive', sans-serif; font-weight:600; font-size:14px; cursor:pointer; transition:0.3s; box-shadow:0 3px 10px rgba(149,117,205,0.3); flex:1; }
        .btn-primary { background: linear-gradient(135deg, #ff9ec0 0%, #9575cd 100%); }
        .btn-secondary { background: linear-gradient(135deg, #9575cd 0%, #7986cb 100%); }
        .btn-danger { background: linear-gradient(135deg, #ff6b6b 0%, #ff5252 100%); }
        .btn-query { background: linear-gradient(135deg, #4CAF50 0%, #45a049 100%); }
...
<body>
    <form id="form1" runat="server">
        ...
        <div class="main-content">
            <h2 class="page-title">Vet Management</h2>

            <div class="container">
                <div class="crud-form">
                    <asp:Label runat="server" ID="lblMessage"></asp:Label>
                    <asp:HiddenField runat="server" ID="hdnVetId" />
                    ...
                    <div class="form-group">
                        <label for="txtShift">Shift</label>
                        <asp:TextBox runat="server" ID="txtShift" Placeholder="Shift"></asp:TextBox>
                    </div>
                    <div class="form-group">
                    ...
                    <div class="button-group">
                        <asp:Button runat="server" ID="btnAddVet" Text="Add Vet" CssClass="btn-primary" OnClick="btnAddVet_Click" />
                        <asp:Button runat="server" ID="btnUpdateVet" Text="Update Vet" CssClass="btn-secondary" OnClick="btnUpdateVet_Click" />
                        <asp:Button runat="server" ID="btnClear" Text="Clear" CssClass="btn-danger" OnClick="btnClear_Click" />
                    </div>
                </div>

                <div class="image-box">
                    <img src="https://i.pinimg.com/1200x/b0/72/6b/b0726b5d548c255417432c82741e5428.jpg" alt="Vet" />
                </div>
            </div>
            ...
            <div class="query-section">
                ...
                <div class="query-examples">
                    ...
                    <code>SELECT * FROM Vet</code>
                    <code>SELECT Fname, Lname, Shift FROM Vet WHERE Shift='Morning'</code>
                    <code>SELECT v.Vet_id, v.Fname, v.Shift, e.Email FROM Vet v LEFT JOIN Vet_Email e ON v.Vet_id=e.Vet_id</code>
                    <code>SELECT Shift, COUNT(*) as Total FROM Vet GROUP BY Shift</code>
                </div>
            </div>
            ...
            <div class="table-container">
                ...
                <asp:GridView runat="server" ID="gvVet" AutoGenerateColumns="False" OnRowCommand="gvVet_RowCommand">
                    <Columns>
                        <asp:BoundField HeaderText="ID" DataField="Vet_id" />
                        ...
                        <asp:TemplateField HeaderText="Actions">
                            <ItemTemplate>
                                <div class="action-buttons">
                                    <asp:LinkButton runat="server" CommandName="EditVet" CommandArgument='<%# Eval("Vet_id") %>' Text="Edit" CssClass="btn-edit" />
                                    <asp:LinkButton runat="server" CommandName="DeleteVet" CommandArgument='<%# Eval("Vet_id") %>' Text="Delete" CssClass="btn-delete" OnClientClick="return confirm('Are you sure you want to delete this vet?');" />
                                </div>
                            </ItemTemplate>
                        </asp:TemplateField>
                    </Columns>
